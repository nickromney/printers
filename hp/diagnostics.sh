...
Options:
  ...
  --soft-reset-pjl      Send a soft PJL reset over port 9100 before diagnostics.
  --experimental-clear-jobs
                        Try to clear printer-side processing jobs via HP Jobs API,
                        then fall back to a soft PJL reset if needed.
  --cancel-connecting   Disable HP web services/ePrint to clear an HP Connected
                        or Instant Ink "connecting" panel state.
...
Examples:
  ...
  ./diagnostics.sh --soft-reset-pjl --save-raw
  ./diagnostics.sh --experimental-clear-jobs --save-raw
  ./diagnostics.sh --cancel-connecting --save-raw
EOF
}
...
send_soft_reset_pjl() {
  ...
  have nc || die "nc is required for --soft-reset-pjl"
...
extract_product_error_log() {
  ...
  printf '%s\n' "$xml" | awk '
    /<pldyn:ErrorLog>/ {
      in_log = 1
      sub(/.*<pldyn:ErrorLog>[[:space:]]*/, "")
      if ($0 != "") {
        print
      }
      next
    }
    in_log && /<\/pldyn:ErrorLog>/ {
      sub(/[[:space:]]*<\/pldyn:ErrorLog>.*/, "")
      if ($0 != "") {
        print
      }
      in_log = 0
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --host)
      ...
      ;;
    --community)
      [ "$#" -ge 2 ] || die "--community requires a value"
      COMMUNITY="$2"
      shift 2
      ;;
    --save-raw)
...
if [ "$SEND_SOFT_RESET_PJL" -eq 1 ]; then
  [ -n "$ENDPOINT_HOST" ] || die "--soft-reset-pjl requires a resolved printer host"
...
if [ "$CANCEL_CONNECTING" -eq 1 ]; then
  [ -n "$ENDPOINT_HOST" ] || die "--cancel-connecting requires a resolved printer host"
